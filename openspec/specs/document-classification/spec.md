...
The system SHALL NOT infer or assign a `document_type` tag automatically during PDF ingestion. A `document_type` tag is only present on an ingested document if the user explicitly included it among the upload's tags.
